...
# The GNU make manual seems to use the words variable and macro interchangeably. But I will only call them variables for simplicity.
# The variable in the makefiles can be dereferenced using $(var) or ${var}, but mostly parentheses are used.

# We can assign values to variable with '=' or ':='

# 1. = (Lazy Assignment):
# When we use '=', it represents a lazy assignment.
# The right-hand side is evaluated lazily, meaning the assignment is made but the assigned string is not evaluated.
# It will be evaluated when when the variable is actually used.
# For example, "MAKE_DEPEND := $(CC)" will set value of "MAKE_DEPEND" as "$(CC)"
...
# For example, "MAKE_DEPEND := $(CC)" will set value of "MAKE_DEPEND" as "gcc", assuming current value of CC was gcc.
# Now whenever MAKE_DEPEND is referenced, it will be evaluated to the value "gcc", even if later CC is assigned "clang" or some other value.
...
# In $(shell date +'%N'), 'date' is the shell command to show date, and '%N' specifies to only display nanoscond part of time.
